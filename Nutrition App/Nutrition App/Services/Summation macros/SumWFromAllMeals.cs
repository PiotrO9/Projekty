...
{
    public static class SumWFromAllMeals
    {
        public static float SumWFromAllMealsMethod(MainPageViewModel mainPageViewModel)
        {
            float sum = 0;
            ...
            foreach (var collection in listOfMealsCollections)
            {
                ...
                {
                    if (float.TryParse(meal.W, out float temp))
                    {
                        sum += temp;
                    }
                }
